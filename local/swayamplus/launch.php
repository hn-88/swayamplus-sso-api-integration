...
// 1. Temporarily stash the enrollmentId (and target URL, so we can recover
// the course ID on the other side of the OAuth2 round trip) in the session.
if (!empty($enrollmentid)) {
    $SESSION->swayam_pending_enrollment = $enrollmentid;
    $SESSION->swayam_pending_wanturl = $wantsurl->out(false);
}
